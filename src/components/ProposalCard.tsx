  ...
  return (
    <section
      className={`rounded-md p-4 border ${
        p.needsVote ? 'border-amber-500' : 'border-zinc-800'
      }`}
    >
      <header className="flex items-center justify-between">
        <h3 className="font-semibold">{p.title}</h3>
        {p.needsVote && (
          <span className="text-amber-500 text-xs">Vote needed</span>
        )}
      </header>
      <p className="text-xs opacity-70">Created: {created}</p>
      <div className="mt-3 flex gap-2">
        <button
          onClick={() => p.onVote('approve')}
          className="px-2 py-1 bg-green-600 rounded"
        >
          ✅ Approve
        </button>
        <button
          onClick={() => p.onVote('reject')}
          className="px-2 py-1 bg-red-600 rounded"
        >
          ❌ Reject
        </button>
        <button
          onClick={() => p.onVote('abstain')}
          className="px-2 py-1 bg-zinc-600 rounded"
        >
          ⚪ Abstain
        </button>
      </div>
